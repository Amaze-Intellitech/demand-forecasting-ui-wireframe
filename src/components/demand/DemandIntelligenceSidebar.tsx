import React from 'react';
import { Link, useNavigate } from 'react-router-dom';
import {
  LayoutDashboard,
  Radio,
  TrendingUp,
  Box,
  Boxes,
  SlidersHorizontal,
  Network,
  AlertTriangle,
  Sparkles,
  Cpu,
} from 'lucide-react';
import aitekLogo from '../../assets/aitek_logo.png';

export interface DemandIntelligenceSidebarProps {
  activeTab: string;
  onSelectTab?: (tabId: string) => void;
}

export const DemandIntelligenceSidebar: React.FC<DemandIntelligenceSidebarProps> = ({
  activeTab,
  onSelectTab,
}) => {
  const navigate = useNavigate();

  const handleNavClick = (id: string) => {
    onSelectTab?.(id);
    const routeMap: Record<string, string> = {
      executive: '/solutions/demand-intelligence/executive',
      'demand-sensing': '/solutions/demand-intelligence/demand-sensing',
      forecast: '/solutions/demand-intelligence/forecast',
      drivers: '/solutions/demand-intelligence/drivers',
      inventory: '/solutions/demand-intelligence/inventory',
      scenarios: '/solutions/demand-intelligence/scenarios',
      'supply-capacity': '/solutions/demand-intelligence/supply-capacity',
      'risk-exceptions': '/solutions/demand-intelligence/risk-exceptions',
      copilot: '/solutions/demand-intelligence/copilot',
      'agent-control': '/solutions/demand-intelligence/agent-control',
    };
    if (routeMap[id]) {
      navigate(routeMap[id]);
    }
  };

  const decisionIntelligenceItems = [
    { id: 'executive', label: 'Executive Command Center', icon: LayoutDashboard },
    { id: 'demand-sensing', label: 'Demand Sensing', icon: Radio },
    { id: 'forecast', label: 'Forecast Intelligence', icon: TrendingUp },
    { id: 'drivers', label: 'Driver & Causal Intelligence', icon: Box },
    { id: 'inventory', label: 'Inventory Intelligence', icon: Boxes },
    { id: 'scenarios', label: 'Scenario & Decision Twin', icon: SlidersHorizontal },
    { id: 'supply-capacity', label: 'Supply & Capacity Optimization', icon: Network },
    { id: 'risk-exceptions', label: 'Risk & Exception Center', icon: AlertTriangle },
    { id: 'copilot', label: 'AI Decision Copilot', icon: Sparkles },
  ];

  const operationsItems = [
    { id: 'agent-control', label: 'Agent Control Center', icon: Cpu },
  ];

  return (
    <aside className="w-full md:w-64 lg:w-68 bg-surface border-r border-border flex flex-col justify-between z-20 flex-shrink-0 relative overflow-hidden select-none">
      {/* Top Header & Navigation Links */}
      <div className="relative z-10 p-5">
        {/* Top AITEK Logo */}
        <div className="pt-1 pb-5 px-1 flex items-center justify-start">
          <Link to="/solutions" className="block focus:outline-none">
            <img
              src={aitekLogo}
              alt="AITEK"
              className="h-16 sm:h-20 w-auto object-contain hover:opacity-90 transition-opacity"
            />
          </Link>
        </div>

        {/* Primary Group: DECISION INTELLIGENCE */}
        <nav className="space-y-1 pt-1">
          {decisionIntelligenceItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;

            return (
              <button
                key={item.id}
                type="button"
                onClick={() => handleNavClick(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-primary text-white shadow-sm'
                    : 'text-body hover:text-deep hover:bg-muted'
                }`}
              >
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-white' : 'text-subtle'}`} />
                <span className="truncate text-left">{item.label}</span>
              </button>
            );
          })}
        </nav>

        {/* Secondary Group Divider: OPERATIONS & GOVERNANCE */}
        <div className="pt-5 pb-1.5">
          <div className="h-[1px] bg-border mb-2.5" />
          <span className="px-3 text-[10px] font-bold uppercase tracking-wider text-subtle">
            Operations & Governance
          </span>
        </div>

        {/* Secondary Group Navigation */}
        <div className="space-y-1">
          {operationsItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
              <button
                key={item.id}
                type="button"
                onClick={() => handleNavClick(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-primary text-white shadow-sm'
                    : 'text-body hover:text-deep hover:bg-muted'
                }`}
              >
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-white' : 'text-subtle'}`} />
                <span className="truncate text-left">{item.label}</span>
              </button>
            );
          })}
        </div>
      </div>

      {/* Bottom Brand Statement */}
      <div className="relative z-10 p-5 pt-3 border-t border-border">
        <div className="w-7 h-[2px] bg-primary mb-2.5" />
        <h4 className="text-sm font-bold text-deep leading-tight tracking-tight">
          Turning<br />
          Data into<br />
          Decisions
        </h4>
        <p className="text-xs text-subtle leading-relaxed font-normal mt-1.5">
          A Smarter,<br />
          More Resilient<br />
          Tomorrow
        </p>
      </div>
    </aside>
  );
};
